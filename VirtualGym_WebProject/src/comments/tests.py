...
class UITestCase(TestCase):
	...
	def test_add_comment(self):
		try:
			...
			title = self.browser.find_element_by_partial_link_text('Side').text
			self.assertEqual('Side' in title, True)
			title = self.browser.find_element_by_partial_link_text('Side').click()
			if(self.browser.find_element_by_partial_link_text('Sign In')):
				...
				self.assertEqual('Side' in check, True)
