...
class UITestCase(TestCase):
    ...
    def test_create_exercise(self):
        try:
            ...
            action.move_to_element_with_offset(annotation, 300, 300)
            action.click()
            action.perform()
            time.sleep(2)
            ...
            add = self.browser.find_element_by_xpath("//*[@id='annotation_submit']").click()
            time.sleep(2)
            # since we used Ajax for annotation
            # in test we want to reload the page to see if the annotaion still there
            self.browser.refresh()
            annotation = self.browser.find_element_by_xpath("//*[@id='annotation_vid']").click()
            close = self.browser.find_element_by_xpath("//*[@id='annotation_closeBtn']").click()
            time.sleep(10)
    ...
    def test_search_by_tag(self):
        try:
            ...
            searchTag.send_keys("Triceps")
            searchTag.click()
            searchButton = self.browser.find_element_by_id("searchButton")
            searchButton.click()
            self.assertIn("http://localhost:8000/viewProfile/?q=Triceps",self.browser.current_url)
    ...
    def test_cross_reference(self):
        try:
            ...
            self.assertTrue("Triceps" in cross)
            ...
            self.assertTrue("Triceps" in cross_exe)
        except:
