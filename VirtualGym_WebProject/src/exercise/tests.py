...
#     # this test takes time to run around 20s
	...
	# 		tag_text = self.browser.find_element_by_xpath('//*[@id="tabs"]/div[4]/div/input[1]')
 ...
 #    # test if user can view accepted exercise list
	...
	# 		tag = self.browser.find_element_by_partial_link_text('test').text
	# 		self.assertEqual('test' in tag, True)
	# 		tag = self.browser.find_element_by_partial_link_text('test').click()
...
#     # test if user can search exercise by annotation
	...
	# 		searchAnnotation.send_keys("anno")
	# 		searchAnnotation.send_keys(Keys.ENTER)
	# 		time.sleep(1)
	# 		self.assertIn("http://localhost:8000/viewProfile/?q=anno",self.browser.current_url)
...
#   # test if cross reference shows correctlly
	...
	# 		self.browser.get("http://localhost:8000/viewProfile/")
	# 		self.assertIn("http://localhost:8000/viewProfile/", self.browser.current_url)
	# 		exercise1 = self.browser.find_element_by_xpath("/html/body/div/div[3]/div/div/section/div/div[3]/div/h4/a").click()
	...
	# 		self.assertTrue("test" in cross_exe)
