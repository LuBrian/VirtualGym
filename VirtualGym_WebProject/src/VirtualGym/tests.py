...
def videoSetUp():
	harrbra = U.MyUsers.objects.get(username = 'harrbra')
	E.Videos.objects.create(videoFile = "Video Test", exercisePosterId = harrbra, annotations = "Annotation Test")
...
class ExerciseTestCase(TestCase):
	...
	def test_exercise_tester(self):
		...
		self.assertNotEqual(exercise.exerciseDate.month, currentMonth-1)
...
class AnnotationTestCase(TestCase):
	...
	def test_annotation(self):
		annotate = E.Videos.objects.get(annotations = 'Annotation Test')

		self.assertEqual("Annotation Test" in annotate.annotations, True)
